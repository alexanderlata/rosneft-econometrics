...
- [t-тест](#t-тест)
  - [approve equation \#1](#approve-equation-1-1)
    - [t test of coefficients (OLS Standard
      Errors)](#t-test-of-coefficients-ols-standard-errors)
    - [t test of coefficients (Robust Standard Errors:
      HC3)](#t-test-of-coefficients-robust-standard-errors-hc3)
  - [approve equation \#2](#approve-equation-2-1)
    - [t test of coefficients (OLS Standard
      Errors)](#t-test-of-coefficients-ols-standard-errors-1)
    - [t test of coefficients (Robust Standard Errors:
      HC3)](#t-test-of-coefficients-robust-standard-errors-hc3-1)
  - [labour force equation \#1](#labour-force-equation-1-1)
    - [t test of coefficients (OLS Standard
      Errors)](#t-test-of-coefficients-ols-standard-errors-2)
    - [t test of coefficients (Robust Standard Errors:
      HC3)](#t-test-of-coefficients-robust-standard-errors-hc3-2)
  - [labour force equation \#2](#labour-force-equation-2-1)
    - [t test of coefficients (OLS Standard
      Errors)](#t-test-of-coefficients-ols-standard-errors-3)
    - [t test of coefficients (Robust Standard Errors:
      HC3)](#t-test-of-coefficients-robust-standard-errors-hc3-3)
...
- [F-тест: совместная значимость](#f-тест-совместная-значимость)
  - [approve equation \#1](#approve-equation-1-3)
    - [Гипотеза H₀: $\beta_{appinc} = \beta_{appinc^2} = 0$
      (α=0.05)](#гипотеза-h₀-beta_appinc--beta_appinc2--0-α005)
    - [Гипотеза H₀: $\beta_{unem} = \beta_{dep} = \beta_{married} = 0$
      (α=0.01)](#гипотеза-h₀-beta_unem--beta_dep--beta_married--0-α001)
- [Прогноз](#прогноз)
...
- [Вопросы адекватности модели](#вопросы-адекватности-модели)
  - [approve equation — график фактических и прогнозируемых
    значений](#approve-equation--график-фактических-и-прогнозируемых-значений)
  - [labour force equation — график фактических и прогнозируемых
    значений](#labour-force-equation--график-фактических-и-прогнозируемых-значений)
...
Рисунок 2: Гистограмма логарифмированного дохода семьи
($\log(\textrm{FAMINC})$) с наложением нормальной кривой плотности.
...
**Коэффициенты, значимые на уровне 1% (по классическим стандартным
ошибкам):** `mortno`, `married`.

**Коэффициенты, значимые на уровне 1% (по робастным стандартным ошибкам
HC3):** `mortno`.
...
гипотезы ($H_0$), согласно которой данный фактор не влияет на зависимую
переменную (истинный коэффициент равен нулю).

- `***` : **$p < 0.01$** (от 0 до 0.01) — *высокая значимость*. Нулевая
  гипотеза отвергается на уровне значимости 1%.
- `**` : **$0.01 \le p < 0.05$** — *стандартная значимость*. Коэффициент
  значим на 5% уровне. Это самый распространенный порог для
  подтверждения гипотез в большинстве исследований.
- `*` : **$0.05 \le p < 0.1$** — *маргинальная (слабая) значимость*.
  ...
  тенденцию».
- `` (пустое место) : **$p \ge 0.1$** (от 0.1 до 1) — *статистически не
  значимо*. У нас нет достаточных оснований утверждать, что переменная
...
**Коэффициенты, значимые на уровне 5% (по классическим стандартным
ошибкам):** `WE`, `np.log(FAMINC)`.

**Коэффициенты, значимые на уровне 5% (по робастным стандартным ошибкам
HC3):** `WE`, `np.log(FAMINC)`.
...
![](list21-LPM_files/figure-commonmark/cell-46-output-1.png)
...
![](list21-LPM_files/figure-commonmark/cell-47-output-1.png)
